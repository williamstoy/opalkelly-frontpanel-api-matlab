...
AXI4liteWriteAndConfirm(okobj, registerSpace('SYSREF Handling')         , '00000001'); % 0x010: Sysref always (TB uses 00000001) prev: 00010001
...
% READ ALL REGISTERS
okobj.verbose = true;
registers = {'Version', 'Reset', 'ILA Support', 'Scrambling', 'SYSREF Handling', 'Test Modes', 'Link Error Status', 'Octets per Frame', 'Frames per Multiframe', 'Lanes in Use', 'Subclass Mode', 'RX Buffer Delay', 'Error Reporting', 'Sync Status', 'Debug Status', 'ILA Config Data 0', 'ILA Config Data 1', 'ILA Config Data 2', 'ILA Config Data 3', 'ILA Config Data 4', 'ILA Config Data 5', 'ILA Config Data 6', 'ILA Config Data 7', 'Test Mode Error Count', 'Link Error Count', 'Test Mode ILA Count', 'Test Mode Multiframe Count', 'Buffer Adjust'};
for i = 1:length(registers)
    okobj.DisplayComment(sprintf('Reading Register: %s', registers{i}));
    AXI4liteRead(okobj, registerSpace(registers{i}));
end

okobj.DisplayComment(sprintf('Reading Register: %s Lane %i', 'ILA Config Data 0', 0));
AXI4liteRead(okobj, '800');
okobj.DisplayComment(sprintf('Reading Register: %s Lane %i', 'ILA Config Data 0', 1));
AXI4liteRead(okobj, '840');
okobj.DisplayComment(sprintf('Reading Register: %s Lane %i', 'ILA Config Data 0', 2));
AXI4liteRead(okobj, '880');
okobj.DisplayComment(sprintf('Reading Register: %s Lane %i', 'ILA Config Data 0', 3));
AXI4liteRead(okobj, '8C0');

okobj.SetWireInValue(vThresholdAddress, 2^5, wireInMask);
...
function output = queryBits(decValue, bitIndicies)
    binArray = flipud(int2bit(decValue, 32));
